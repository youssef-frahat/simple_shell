...
/**
 * print_errors - prints when a syntax error is found
 *
 ...
 * Return: no return
 */
void print_errors(data_shell *datash, char *input, int i, int bool)
{
...
/**
 * check_errors - intermediate function to
 ...
 * Return: 1 if there is an error. 0 in other case
 */
int check_errors(data_shell *datash, char *input)
{
	int begin = 0;
	int f_char = 0;
	int i = 0;
	...
	if (f_char == -1)
	{
		print_errors(datash, input, begin, 0);
		return (1);
	}
	...
	if (i != 0)
	{
		print_errors(datash, input, begin + i, 1);
		return (1);
	}
